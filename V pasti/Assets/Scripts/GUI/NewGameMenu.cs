...
public class NewGameMenu : MonoBehaviour
{
	...
	public void PlayPressed()
	{
		...
		// Nova postava zatim nema zadne obleceni, prazdny item je -1
		string name = transform.FindChild ("PlayerName").FindChild ("Text").GetComponent<Text> ().text;
		sqlQuery = "INSERT INTO DressedItems (PlayerID,Head,Chest,Shoulders,Hands,Legs,Feet,Sword,Staff,Shield) SELECT ID, -1,-1,-1,-1,-1,-1,-1,-1,-1 from Players WHERE Players.playerName = '" + name + "';";
		command = connection.CreateCommand();
		command.CommandText = sqlQuery;
		command.ExecuteNonQuery();
		command.Dispose();
